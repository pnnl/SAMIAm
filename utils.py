...
# def show_box(box, ax, label="",linecolor=[],edgecolor='black'):
#     x0, y0 = box[0], box[1]
#     w, h = box[2] - box[0], box[3] - box[1]
#     ax.add_patch(plt.Rectangle((x0, y0), w, h, edgecolor=edgecolor, facecolor=(0,0,0,0), lw=5))
#     if label:
...
# def emd(pred_a, pred_b):
#     """
#     Compute Earth Mover's Distance between two probability distributions.

#     Parameters:
#     - pred_a: First probability distribution (PyTorch tensor).
#     - pred_b: Second probability distribution (PyTorch tensor).

#     Returns:
#     - emd_value: Earth Mover's Distance between pred_a and pred_b.
#     """
#     # Ensure the input distributions are normalized to probabilities
#     pred_a = pred_a / torch.sum(pred_a)
#     pred_b = pred_b / torch.sum(pred_b)

#     # Compute cost matrix
#     cost_matrix = torch.abs(pred_a.unsqueeze(1) - pred_b.unsqueeze(0))

#     # Solve the linear sum assignment problem (minimum cost bipartite matching)
#     row_ind, col_ind = torch.tensor(torch.optim.lr_scheduler._LRScheduler.linear_sum_assignment(cost_matrix))

#     # Calculate the EMD value
#     emd_value = cost_matrix[row_ind, col_ind].sum()

#     return emd_value.item()

# def euclidean_distance(pred_a, pred_b):
#     """
#     Compute the Euclidean distance between two vectors.

#     Parameters:
#     - pred_a: First vector (list or array).
#     - pred_b: Second vector (list or array).

#     Returns:
#     - euclidean_distance: Euclidean distance between pred_a and pred_b.
#     """
...
#     assert len(pred_a) == len(pred_b), "Vectors must have the same length"

#     # Compute the Euclidean distance (Minkowski distance with p=2)
#     euclidean_distance = np.linalg.norm(pred_a - pred_b)
